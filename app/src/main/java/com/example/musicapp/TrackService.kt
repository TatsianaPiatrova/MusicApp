...
class TrackService : MediaBrowserServiceCompat() {
    ...
    private var _audioList: TrackList? = null
    private val audioList: TrackList by lazy { getSingleAudioList() }
    private var id: Int = 0


    private fun getSingleAudioList(): TrackList {
        return if (_audioList == null) {
            val catalog = TrackList(this.applicationContext)
            _audioList = catalog
            audioList
        } else {
            ...
            if (!audioFocusRequested) {
                ...
                val audioFocusResult = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    ...
                    audioManager?.requestAudioFocus(
                        this@TrackService.audioFocusChangeListener,
    ...
    private fun refreshNotificationAndForegroundStatus(playbackState: Int) {
        when (playbackState) {
            ...
            PlaybackStateCompat.STATE_PAUSED -> {
                NotificationManagerCompat.from(this@TrackService).notify(
                    NOTIFICATION_ID,
                    getNotification(playbackState)
                )
